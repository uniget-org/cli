...
type Config struct {
	...
	cacheDir          string `arg:"env=UNIGET_CACHEDIRECTORY"`
...
func (config *Config) Update() error {
	...
	if !config.user {
		...
		config.cacheDir = config.cacheDirectory + "/" + config.fileCacheDirectoryName
	...
	case "file":
		...
		fileCacheDir := config.prefix + "/" + config.cacheDir
...
func (config *Config) Debug() {
	...
	logging.Debugf("cache-directory: %s", config.cacheDir)
