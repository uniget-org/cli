...
func GetPlatformManifest(ctx *context, rc *regclient.RegClient, r *ref.Ref, alt_arch string) (manifest.Manifest, error) {
		...
		if mi, ok := m.(manifest.Indexer); ok {
			...
			for _, manifest := range manifests {
				...
				if manifest.Platform.Architecture == alt_arch {
					platformImage := fmt.Sprintf("%s@%s", r, manifest.Digest)
